...
#include "hal/uart_hal.h"
  ...
  void dmx_hal_init(uart_hal_context_t *hal)
  {
    ...
    uart_ll_set_baudrate(hal->dev, DMX_TYP_BAUD_RATE, REF_CLK_FREQ);
  ...
  {
    uart_hal_set_baudrate(hal, baud_rate, REF_CLK_FREQ);
  }
  ...
  {
    uint32_t baud_rate;
    uart_hal_get_baudrate(hal, &baud_rate, REF_CLK_FREQ);
    return baud_rate;
  }
